...
export const getContents = async (path = '/'): Promise<ContentsWithRoot> => {
	...
	const filterHidden = (node: Node) =>
		// We need to hide files from hidden directories in the root if not configured to show
		path === '/'
		&& !store.userConfig.show_hidden
		&& node.dirname.split('/').some((dir) => dir.startsWith('.'))
